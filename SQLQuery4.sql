...
WHERE customers.age>=30 AND customers.customer_id= orders.customer_id


--5. What is the total revenue generated by each product category?

SELECT products.category,SUM(products.price) AS Total_revenue
FROM products, baskets, orders
WHERE products.product_id=baskets.product_id AND baskets.order_id= orders.order_id
GROUP BY products.category 

--6. What is the average price of products in the 'food' category?

SELECT AVG(products.price) AS Avg_foodprice
FROM products
WHERE products.category= 'food'

--7. How many orders were made in each sales channel (sales_channel column) in the orders table?

SELECT orders.sales_channel, COUNT(orders.order_id)
FROM orders
...
SELECT MAX(orders.date_shop) AS latest_orderdate
FROM orders, customers
WHERE customers.can_email= 'yes' AND customers.customer_id= orders.customer_id

--9. What is the name of the country with the highest number of orders?

SELECT TOP 1 country.country_name, COUNT(orders.order_id) AS order_count
FROM country, orders
...
SELECT products.category ,AVG(customers.age) AS average_age
FROM customers,products, baskets, orders
WHERE products.category= 'vitamins' AND
products.product_id= baskets.product_id AND
